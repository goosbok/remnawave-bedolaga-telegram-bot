...
async def _check_and_recover_pending_purchase(
    ...
    if match is None:
        ...
        # Для остальных методов это нормальный путь — гость нажал «Оплатить», но
        # ещё не завершил оплату у провайдера. Polling задача проверяет каждые N
        # секунд для каждой PENDING покупки, поэтому debug-лог здесь не пишется —
        # реальные ошибки (amount mismatch и т.д.) логируются на уровне ERROR ниже.
        return False
